...
const PostGenerator: React.FC<PostGeneratorProps> = ({ 
  ...
  const performGeneration = async (
    ...
    for (const p of targetPlatforms) {
      ...
      try {
        const content = await generateContent(storeProfile, config, isPro);
  ...
  const handleRefine = async (groupIndex: number, itemIndex: number) => {
    ...
    try {
      ...
      const newContent = await refineContent(storeProfile, group.config, currentContent, refineText);
                ...
                {(Object.values(Platform) as Platform[]).map((p) => {
                  ...
                  return (
                    <button
                      ...
                      disabled={isDisabled}
                      className={`flex-1 py-3.5 md:py-3 text-base md:text-sm font-bold rounded-xl transition-all duration-300 relative ${
                        isSelected 
                  ...
                  {isMap
                    ? GMAP_PURPOSES.map((opt) => (
                        <button
                          ...
                          className={`group relative flex flex-col items-center justify-center gap-2 md:gap-3 py-4 md:py-3 px-3 md:px-3 rounded-2xl border transition-all duration-200 overflow-hidden ${
                            gmapPurpose === opt.value
                        ...
                        >
                          <div className={`p-3 md:p-3 rounded-full transition-all duration-300 ${
                            gmapPurpose === opt.value
              ...
              <div className="space-y-3 relative" ref={styleRef}>
                ...
                <div className={`bg-white p-1 rounded-xl shadow-sm border border-gray-100 flex ${!isLoggedIn ? 'opacity-50 pointer-events-none' : ''}`}>
                    {TONES.map(t => (
                        ...
                        disabled={!isLoggedIn}
                        className={`flex-1 py-3.5 md:py-2 text-sm md:text-xs font-bold rounded-lg transition-all ${
                            tone === t.value 
                ...
                {!isMap && (
                  <div className="grid grid-cols-2 gap-2">
                     <button
                        ...
                        disabled={!isLoggedIn}
                        className={`p-2.5 rounded-xl border flex flex-col items-start gap-2 relative transition-all ${
                            includeEmojis 
                ...
                {platforms.includes(Platform.X) && isLoggedIn && (
                  <div className="bg-black/5 p-3 rounded-xl flex items-center justify-between border border-black/5">
                    ...
                    >
                      <span
                        className={`${
                          xConstraint140 ? 'translate-x-5' : 'translate-x-1'
                    ...
                    <div key={gIdx} className="space-y-4">
                      <div className="flex items-center gap-2 px-2">
                        <span className={`p-1.5 rounded-lg text-white ${
                          group.platform === Platform.X ? 'bg-black' : 
                              ...
                              {group.platform === Platform.Instagram && storeProfile.instagramFooter && (
                                  <div className="mt-4 bg-gradient-to-r from-pink-50 to-purple-50 p-3 rounded-xl flex items-center justify-between border border-pink-100">
                                      ...
                                      >
                                        <span
                                          className={`${
                                            res.includes(storeProfile.instagramFooter) ? 'translate-x-5' : 'translate-x-1'
                                          } inline-block h-3 w-3 transform rounded-full bg-white transition-transform`}
                                        />
                                      </button>
                                  </div>
                              ...
                              <div className="flex flex-col mt-auto gap-2">
                                  <div className="flex items-center gap-2">
                                      <button
                                        ...
                                        className={`flex-1 py-4 md:py-3 border rounded-xl text-base md:text-sm font-bold transition-colors flex items-center justify-center gap-2 ${
                                          !isLoggedIn 
                                  ...
                                  <button
                                      ...
                                      disabled={!isLoggedIn}
                                      className={`w-full py-4 md:py-3 rounded-xl text-base md:text-sm font-bold transition-all flex items-center justify-center gap-2 shadow-sm ${
                                        !isLoggedIn
