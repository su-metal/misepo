...
export const PWASection = ({ isMobile = false }: { isMobile?: boolean }) => {
    return (
        <section id="pwa" className="py-24 md:py-48 bg-white overflow-hidden">
            <div className="max-w-7xl mx-auto px-6">
                <div className="flex flex-col items-end text-right mb-24">
                    ...
                    <h2 className={`font-black tracking-tighter leading-[0.9] text-[#282d32] ${isMobile ? 'text-5xl' : 'text-7xl md:text-8xl lg:text-[7.5rem]'}`}>
                        場所を選ばない、<br />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { title: "INSTANT START", desc: "ロード時間ゼロ。思いついたその瞬間に投稿を作成できます。" },
                        { title: "APP FEEL", desc: "PWA技術により、ホーム画面からアプリ感覚でサクサク起動。" },
                        { title: "MULTI DEVICE", desc: "店外ではスマホ、バックヤードではPC。全てが同期されます。" },
                        { title: "ALWAYS LATEST", desc: "更新作業は不要。常に最新のAIモデルがあなたを支えます。" }
                    ].map((item, idx) => (
                        <div key={idx} className="bg-slate-50 rounded-[40px] p-10 border border-slate-100 flex flex-col items-start transition-all hover:bg-white hover:shadow-2xl hover:shadow-slate-200/50">
