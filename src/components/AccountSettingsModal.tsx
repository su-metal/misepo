...
const AccountSettingsModal: React.FC<AccountSettingsModalProps> = ({ user, plan, onClose, onLogout }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4 z-[200] animate-in fade-in duration-300" onClick={onClose}>
            <div
                className="bg-[var(--bg-beige)] w-full max-w-lg rounded-[32px] border-[3px] border-black shadow-[8px_8px_0_0_rgba(0,0,0,1)] overflow-hidden animate-in zoom-in-95 duration-300"
            ...
            >
                ...
                <div className="p-6 md:p-8 space-y-8 bg-[var(--bg-beige)]">
                    ...
                    {/* User Profile Card */}
                    <div className="flex items-center gap-5 p-5 bg-white rounded-2xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)]">
                        <div className="relative shrink-0">
                            {user?.user_metadata?.avatar_url ? (
                                ...
                                <div className="w-16 h-16 rounded-xl bg-[var(--lavender)] border-2 border-black flex items-center justify-center text-xl font-black text-black">
                                    {(user?.email?.[0] || 'U').toUpperCase()}
                                </div>
                            )}
                            <div className="absolute -bottom-2 -right-2 w-6 h-6 bg-white rounded-full flex items-center justify-center border-2 border-black shadow-sm z-10">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className={`p-4 rounded-2xl border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] ${isPro ? 'bg-[var(--gold)]/20' : 'bg-white'}`}>
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-xl border-2 border-black flex items-center justify-center shrink-0 shadow-[2px_2px_0_0_rgba(0,0,0,1)] ${isPro ? 'bg-[var(--gold)] text-black' : 'bg-slate-100 text-slate-400'}`}>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {isPro && <span className="text-[9px] bg-black text-[#FFD700] border border-black px-2 py-0.5 rounded-full font-black uppercase tracking-widest">{isTrial ? 'Trialing' : 'Active'}</span>}
                                        </div>
                                ...
                                {!isPaid && !isTrial && (
                                    <a href="/pricing" className="px-4 py-2 bg-black text-white text-[10px] font-black rounded-lg hover:bg-slate-800 transition-all border-2 border-transparent hover:border-black uppercase tracking-widest shrink-0 shadow-[2px_2px_0_0_rgba(0,0,0,1)]">
                                        Upgrade
                                    </a>
                                )}
                            </div>
                    ...
                    <div className="space-y-4">
                        <button
                            ...
                            className="w-full flex items-center justify-between p-4 rounded-xl bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0_0_rgba(0,0,0,1)] active:translate-x-[0px] active:translate-y-[0px] active:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all group disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            <div className="flex items-center gap-4">
                        ...
                        <button
                            onClick={onLogout}
                            className="w-full flex items-center justify-between p-4 rounded-xl bg-white border-2 border-black shadow-[4px_4px_0_0_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] hover:shadow-[6px_6px_0_0_rgba(0,0,0,1)] active:translate-x-[0px] active:translate-y-[0px] active:shadow-[2px_2px_0_0_rgba(0,0,0,1)] transition-all group hover:bg-rose-50"
                        >
                            <div className="flex items-center gap-4">
