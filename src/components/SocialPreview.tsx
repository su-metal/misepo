...
const SocialPreview: React.FC<SocialPreviewProps> = ({ platform, text, storeName }) => {
  ...
  if (platform === Platform.X) {
    return (
      <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden w-full max-w-md mx-auto font-sans text-[15px] leading-normal text-stone-900 shadow-xl shadow-slate-200/50">
            ...
            <div className="mt-0.5 whitespace-pre-wrap break-words text-stone-900">
              ...
              {text.split(/(\s+)/).map((part, i) => {
                if (part.startsWith('#') || part.startsWith('http')) {
                  return <span key={i} className="text-lime whitespace-nowrap">{part}</span>;
                }
                return part;
              })}
            </div>
  ...
  if (platform === Platform.Instagram) {
    return (
      <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden w-full max-w-sm mx-auto font-sans text-sm shadow-xl shadow-slate-200/50">
        {/* Header */}
  ...
  if (platform === Platform.GoogleMaps) {
    return (
      <div className="bg-white border border-slate-100 rounded-2xl p-4 w-full max-w-md mx-auto font-sans shadow-xl shadow-slate-200/50">
