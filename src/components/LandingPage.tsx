...
const StatCard = () => {
    ...
    return (
        <div ref={cardRef} className="md:col-span-8 bg-white rounded-[4rem] p-12 md:p-20 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.08)] border-4 border-white flex flex-col md:flex-row justify-between items-center group overflow-hidden relative gap-12">
            ...
            {/* Background Illustration - Monster Blobs (Cut-out style) */}
            <div className="absolute -bottom-6 -right-6 w-56 h-56 opacity-20 group-hover:opacity-40 transition-opacity transform rotate-6 group-hover:scale-110 duration-700 pointer-events-none z-0">
                <img
                    src="/assets/landing/clay_monster_sns.png"
                    alt=""
                    className="w-full h-full object-contain mix-blend-multiply filter brightness-105 contrast-110"
                />
            </div>
            ...
            <div className="flex-1 relative z-10">
                ...
                <div className="flex items-end gap-2 mb-8">
                    <h2 className="text-8xl md:text-[10rem] font-black text-indigo-950 leading-none tracking-tighter w-[1.2em]">
                        {count}<span className="text-4xl md:text-5xl">%</span>
                    </h2>
                </div>
            ...
            <div className="relative w-48 h-48 md:w-64 md:h-64 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle
                        ...
                        cy="50%"
                        r="45%"
                        className="stroke-lime-400 transition-all duration-100 ease-out"
                        strokeWidth="12"
                        fill="transparent"
                        strokeLinecap="round"
                        style={{
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                    <div className="w-4 h-4 rounded-full bg-lime-400 shadow-[0_0_20px_rgba(163,230,53,1)]" />
                </div>
            </div>
        </div>
    );
...
export default function LandingPage() {
    ...
    // ===== STEP 4: PROCESSING ANIMATION (0.40-0.55) =====
    const step4Progress = Math.min(Math.max((scrollProgress - 0.40) / 0.15, 0), 1);

    // Processing Node
    const processIntro = Math.min(Math.max((scrollProgress - 0.40) / 0.05, 0), 1);
    const processStay = scrollProgress >= 0.45 && scrollProgress <= 0.52 ? 1 : 0;
    const processExit = Math.min(Math.max((scrollProgress - 0.52) / 0.03, 0), 1);
    ...
    // Processing Labels
    const processLabel1Opacity = Math.min(Math.max((scrollProgress - 0.41) / 0.04, 0), 1) * processOpacity;
    const processLabel2Opacity = Math.min(Math.max((scrollProgress - 0.43) / 0.04, 0), 1) * processOpacity;
    const processLabelScale = 0.8 + (processOpacity * 0.2);

    // ===== STEP 5: RESULT INTRODUCTION (0.55-0.65) =====
    const step5Progress = Math.min(Math.max((scrollProgress - 0.55) / 0.10, 0), 1);

    // Step 5 Label: "完成した投稿"
    const step5LabelOpacity = Math.min(Math.max((scrollProgress - 0.56) / 0.04, 0), 1);
    const step5LabelX = 50 * (1 - Math.min(Math.max((scrollProgress - 0.56) / 0.04, 0), 1));
    ...
    // ===== STEP 6: RESULT DETAILS (0.65-0.75) =====
    const step6Progress = Math.min(Math.max((scrollProgress - 0.65) / 0.10, 0), 1);

    // Content cascade (ends by 0.75)
    const resultTitleOpacity = Math.min(Math.max((scrollProgress - 0.66) / 0.03, 0), 1);
    const resultBodyOpacity = Math.min(Math.max((scrollProgress - 0.69) / 0.03, 0), 1);
    const resultTagsOpacity = Math.min(Math.max((scrollProgress - 0.72) / 0.03, 0), 1);

    // Final buffer exit (0.93-1.00) to prevent jarring unpin
    const exitBuffer = Math.min(Math.max((scrollProgress - 0.93) / 0.07, 0), 1);
    const containerOpacity = 1 - (exitBuffer * 0.2);
    const containerScale = 1 - (exitBuffer * 0.02);
    ...
    return (
        <div className="bg-[#5c72ff] min-h-screen text-white font-sans selection:bg-lime-400 selection:text-black">
            ...
            <section className="py-32 relative z-20">
                <div className="container mx-auto px-6 max-w-6xl">
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
                        ...
                        {/* 1. Large Statistical Impact Card (Animated) */}
                        <StatCard />
                        ...
                        <div className="md:col-span-4 bg-lime-400 rounded-[4rem] p-12 shadow-[0_50px_100px_-20px_rgba(163,230,53,0.3)] border-4 border-lime-300 flex flex-col justify-between relative overflow-hidden group">
                            {/* Illustration - Monster AI Robot (Cut-out style) */}
                            <div className="absolute -bottom-6 -right-6 w-56 h-56 md:w-64 md:h-64 transform rotate-6 group-hover:scale-110 group-hover:-rotate-3 transition-transform duration-700 pointer-events-none z-0">
                                <img
                                    src="/assets/landing/clay_monster_ai.png"
                                    alt=""
                                    className="w-full h-full object-contain drop-shadow-2xl mix-blend-multiply filter brightness-105 contrast-110"
                                />
                            </div>
                            <div className="absolute bottom-6 left-6 p-4 opacity-10 group-hover:scale-110 transition-transform z-0">
                                <Icons.Coffee />
                            </div>
                            <div className="relative z-10">
                                <span className="text-[10px] font-black uppercase tracking-[.4em] text-indigo-900/40 mb-8 block font-en">Barrier Zero</span>
                                <h3 className="text-3xl font-black text-indigo-950 tracking-tighter leading-[1.1] mb-8">
                                    教育コストを<br />極限まで削減。
                                </h3>
                                <p className="text-sm font-bold font-serif text-indigo-900/60 leading-relaxed max-w-[200px]">
                        ...
                        <div className="md:col-span-6 bg-white rounded-[4rem] p-12 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.08)] border-4 border-white flex items-center gap-10 group relative overflow-hidden">
                            <div className="absolute -right-6 -bottom-6 w-48 h-48 opacity-20 group-hover:opacity-40 transition-opacity transform -rotate-12 pointer-events-none z-0">
                                <img
                                    src="/assets/landing/clay_monster_sparkle.png"
                                    alt=""
                                    className="w-full h-full object-contain mix-blend-multiply filter brightness-105 contrast-110"
                                />
                            </div>
                            <div className="w-24 h-24 rounded-3xl bg-indigo-50 flex items-center justify-center text-indigo-600 group-hover:rotate-12 transition-transform shadow-inner shrink-0 relative z-10">
                                <Icons.Sparkle />
                            </div>
                            <div className="flex-1 relative z-10">
            ...
            <section id="features" ref={sectionRef} className="relative h-[1200vh]" >
