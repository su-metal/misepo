        ...
        <div className="flex flex-col bg-white">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center gap-4 px-3 sm:px-6 pt-8 pb-4">
                ...
                <div className="flex items-center gap-3 px-5 py-2.5 bg-white rounded-full border border-slate-200 shadow-sm">
                    <span className="text-[10px] font-black text-[#001738] uppercase tracking-[0.2em] leading-none">同時生成</span>
                    <button
                        onClick={onToggleMultiGen}
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-all duration-300 ${isMultiGen ? 'bg-[#E5005A]' : 'bg-slate-200'}`}
                    >
            ...
            <div className="flex flex-col flex-1 gap-8 px-3 sm:px-6 py-6">
                ...
                <div className="w-full shrink-0 flex flex-col gap-6 pr-2 order-1">
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-2 gap-x-4 gap-y-4 p-1">
                            ...
                            <button
                                ...
                                className={`relative py-3 px-4 rounded-[20px] border-2 transition-all flex flex-col items-center gap-2 group shadow-sm
                                    ${!activePresetId
                            ...
                            >
                                ...
                                <span className={`text-[11px] font-black text-center line-clamp-1 leading-tight ${!activePresetId ? 'text-white' : 'text-slate-600'}`}>プレーンAI</span>
                                {!activePresetId && (
                                    <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-[#E5005A] shadow-sm animate-pulse" />
                                )}
                            </button>
                            ...
                            {presets.slice(0, 3).map((preset) => (
                                ...
                                >
                                    ...
                                    <span className={`text-[11px] font-black text-center line-clamp-1 leading-tight ${activePresetId === preset.id ? 'text-white' : 'text-slate-600'}`}>{preset.name}</span>
                                    {activePresetId === preset.id && (
                                        <div className="absolute top-2 right-2 w-2 h-2 rounded-full bg-[#E5005A] shadow-sm animate-pulse" />
                                    )}
                                </button>
                            ...
                            {presets.length < 3 && (
                                <button
                                    ...
                                    className="py-3 px-4 rounded-[20px] border-[3px] border-dashed border-slate-100 bg-slate-50/50 text-slate-300 hover:border-indigo-200 hover:text-indigo-400 transition-all flex flex-col items-center gap-2 group"
                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Tone Selection */}
                        <section>
                            <h3 className="text-[10px] font-black text-[#001738] uppercase tracking-[0.25em] mb-3 px-1">Tone</h3>
                            <div className={`flex flex-row gap-1.5 bg-slate-100/50 p-1 rounded-[20px] border border-slate-200/50 ${activePresetId ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                {TONES.map((t) => (
                                    <button
                                        key={t.id}
                                        onClick={() => onToneChange(t.id)}
                                        disabled={!!activePresetId}
                                        className={`flex-1 py-2.5 px-1 rounded-[16px] text-[10px] font-black transition-all flex items-center justify-center gap-1.5 relative ${tone === t.id
                        ...
                        {!isX && (
                            <section>
                                <h3 className="text-[10px] font-black text-[#001738] uppercase tracking-[0.25em] mb-3 px-1">Length</h3>
                                <div className="flex flex-row gap-1.5 bg-slate-100/50 p-1 rounded-[20px] border border-slate-200/50">
                                    {LENGTHS.map((l) => (
                                        <button
                                            key={l.id}
                                            onClick={() => onLengthChange(l.id)}
                                            className={`flex-1 py-2.5 px-1 rounded-[16px] text-[10px] font-black transition-all flex items-center justify-center gap-1.5 relative ${length === l.id
                                                ? 'bg-[#001738] text-white shadow-lg shadow-navy-900/20'
                                                : 'text-slate-400 hover:text-slate-600 hover:bg-white'
                                                }`}
                                        >
                        ...
                        {isGoogleMaps && (
                            <section className="animate-in fade-in duration-500">
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Rating</h3>
                                <div className="flex justify-center gap-1 bg-slate-50 p-2.5 rounded-[20px] border border-slate-100 shadow-sm">
                                    {[1, 2, 3, 4, 5].map((r) => (
                                        <button
                                            key={r}
                        ...
                        {isX && (
                            <section className="animate-in fade-in duration-500">
                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 px-1">Limit</h3>
                                <button
                                    onClick={() => onXConstraint140Change(!xConstraint140)}
                                    className={`w-full p-3 rounded-[20px] border transition-all flex items-center justify-between group shadow-sm
                                ...
                                >
                                    <span className="text-[10px] font-black">140文字</span>
                                    <div className={`w-4 h-4 rounded-full transition-all flex items-center justify-center ${xConstraint140 ? 'bg-indigo-500' : 'bg-slate-100'}`}>
                        ...
                        {/* Style Options */}
                        {!isGoogleMaps && (
                            <section className="animate-in fade-in duration-500 delay-150 col-span-1 md:col-span-2">
                                <h3 className="text-[10px] font-black text-[#001738] uppercase tracking-[0.25em] mb-3 px-1">Style Options</h3>
                                <div className={`flex flex-row items-center gap-4 bg-white p-3 rounded-[20px] border border-slate-100 shadow-sm ${activePresetId ? 'opacity-50' : ''}`}>
                                    <div className="flex-1 flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            disabled={!!activePresetId}
                                            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-all duration-300 ${activePresetId ? 'cursor-not-allowed' : ''} ${includeEmojis ? 'bg-[#E5005A]' : 'bg-slate-200'}`}
                                        >
                                            <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform duration-300 ${includeEmojis ? 'translate-x-4' : 'translate-x-0.5'}`} />
                                        </button>
                                    </div>
                                    ...
                                    <div className="flex-1 flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            disabled={!!activePresetId}
                                            className={`relative inline-flex h-5 w-9 items-center rounded-full transition-all duration-300 ${activePresetId ? 'cursor-not-allowed' : ''} ${includeSymbols ? 'bg-[#E5005A]' : 'bg-slate-200'}`}
                                        >
                                            <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform duration-300 ${includeSymbols ? 'translate-x-4' : 'translate-x-0.5'}`} />
                                        </button>
                                    </div>
                                </div>
                            </section>
                ...
                <div className="flex-1 flex flex-col gap-8 order-2 min-w-0">
                    <div className="flex-1 bg-white rounded-[40px] lg:rounded-[48px] p-5 lg:p-12 border-2 border-slate-100 flex flex-col group transition-all hover:border-[#001738] shadow-2xl shadow-slate-200/50 relative isolate">
                        ...
                        {/* Canvas Footer */}
                        <div className="mt-10 pt-8 border-t-2 border-slate-50 relative z-10">
                            {/* Additional Instructions Indicator / Field */}
                            {!isPromptExpanded ? (
                                <button
                                    onClick={() => setIsPromptExpanded(true)}
                                    className="flex items-center gap-3 py-2 px-1 text-indigo-500/60 hover:text-indigo-600 transition-colors group mb-4"
                                >
                                    <div className="w-8 h-8 rounded-full bg-indigo-50 flex items-center justify-center group-hover:scale-110 transition-transform">
                                        <AutoSparklesIcon className="w-4 h-4" />
                                    </div>
                                    <span className="text-[11px] font-black uppercase tracking-wider">AIへの指示を追加（任意）</span>
                                </button>
                            ) : (
                                <div className="flex items-center gap-3 bg-white border-2 border-indigo-100 rounded-full pl-5 pr-2 py-2 mb-4 animate-in zoom-in-95 duration-200">
                                    <AutoSparklesIcon className="w-4 h-4 text-indigo-400 shrink-0" />
                                    <input
                                        type="text"
                                        value={customPrompt}
                                        onChange={(e) => onCustomPromptChange(e.target.value)}
                                        placeholder="例：絵文字多めで、テンション高めに..."
                                        className="flex-1 bg-transparent border-none focus:outline-none text-[13px] font-bold text-[#001738] placeholder:text-slate-300"
                                        autoFocus
                                    />
                                    <button
                                        onClick={() => {
                                        ...
                                        className="w-8 h-8 flex items-center justify-center text-slate-300 hover:text-slate-500 hover:bg-slate-50 rounded-full transition-all"
                                    >
                    ...
                    <button
                        ...
                        className={`hidden lg:flex w-full py-8 rounded-[32px] font-black text-2xl tracking-[0.3em] transition-all relative overflow-hidden shadow-2xl group flex-row items-center justify-center
                            ${isGenerating || !inputText.trim()
                                ...
                                : 'bg-[#E5005A] text-white hover:bg-[#C2004D] hover:scale-[1.02] hover:shadow-[#E5005A]/40 active:scale-[0.98]'
                            }`}
                    >
