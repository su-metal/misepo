...
export const LinePreview: React.FC<LinePreviewProps> = ({ text, storeProfile }) => {
    const now = new Date();
    ...
    return (
        <div className="bg-[#7494C0] rounded-2xl p-4 sm:p-6 min-h-[300px] flex flex-col gap-4 font-sans border-2 border-black/10 shadow-inner overflow-hidden">
            ...
            <div className="flex items-start gap-2 sm:gap-3 text-left">
                {/* Avatar */}
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white border border-black/5 flex items-center justify-center shrink-0 shadow-sm overflow-hidden">
                ...
                <div className="flex flex-col gap-1 max-w-[85%]">
                    <span className="text-white text-[10px] sm:text-[11px] font-black pl-1 drop-shadow-sm">
                        {storeProfile.name || 'Store Name'}
                    </span>
                    <div className="flex items-end gap-1.5">
                        {/* Bubble */}
                        <div className="relative bg-[#8EE071] text-black px-3 sm:px-4 py-2 sm:py-2.5 rounded-[18px] rounded-tl-none shadow-sm border border-[#7BC560]/40">
                            {/* Triangle hook */}
                        ...
                        {/* Time */}
                        <div className="flex flex-col text-[#f8f8f8] text-[9px] sm:text-[10px] font-bold leading-tight pb-0.5">
                            <span>{timeStr}</span>
                        </div>
                    </div>
                </div>
