...
export const MobileCalendarOverlay: React.FC<MobileCalendarOverlayProps> = ({
    ...
    React.useEffect(() => {
        if (isOpen && !hasFetched) {
            // Initial fetch: Feb 2026 (or whatever current state is)
            fetchTrends(currentYear, currentMonth);
        }
    }, [isOpen, hasFetched, currentYear, currentMonth]);

    // Check if we need to fetch more data when switching months
    React.useEffect(() => {
        if (!isOpen) return;

        // Check if current month has data in cache
        ...
        // If no data and not currently loading, and we haven't tried for this specific month yet
        if (!hasDataForMonth && !isLoading) {
            fetchTrends(currentYear, currentMonth);
        }
    }, [currentYear, currentMonth, trendCache, isOpen, isLoading]); // Removed hasFetched from deps to rely on trendCache check
            ...
            <div className="relative w-[90%] max-w-sm bg-white/10 backdrop-blur-2xl border border-white/20 rounded-[40px] shadow-2xl p-6 overflow-hidden animate-in zoom-in-95 duration-300">
                    ...
                    <div className="flex flex-col">
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className="p-1 -ml-1 hover:bg-white/10 rounded-full transition-colors active:scale-95 disabled:opacity-0 disabled:pointer-events-none"
                            >
                            ...
                            <button
                                ...
                                disabled={currentYear === baseDate.getFullYear() && currentMonth >= (baseDate.getMonth() + 2)}
                                className="p-1 hover:bg-white/10 rounded-full transition-colors active:scale-95 disabled:opacity-0 disabled:pointer-events-none"
                            >
                ...
                <div className="grid grid-cols-7 gap-1 mb-6 min-h-[240px]">
                    ...
                    {isLoading && trendCache.length === 0 ? (
                        <div className="col-span-7 h-full flex flex-col items-center justify-center text-white/50 py-10">
                            <SparklesIcon className="w-8 h-8 animate-spin mb-2 text-[var(--plexo-yellow)]" />
                            <span className="text-xs font-bold tracking-widest animate-pulse">AI FORECASTING...</span>
                        </div>
                    ) : (
                ...
                <div className={`transition-all duration-300 ${currentEvent || selectedDate ? 'opacity-100 translate-y-0' : 'opacity-50 translate-y-4'}`}>
                    {currentEvent ? (
                        ...
                        <div className="bg-white/5 rounded-[24px] p-5 border border-white/10 flex items-center justify-center h-[140px] text-white/30 text-xs font-bold uppercase tracking-widest">
                            {isLoading ? "AI Analysis in progress..." : (selectedDate ? "特になし" : "日付を選択")}
                        </div>
                    )}
                </div>
            </div>
