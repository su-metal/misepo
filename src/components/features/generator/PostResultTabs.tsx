...
export const PostResultTabs: React.FC<PostResultTabsProps> = ({
        ...
        switch (platform) {
            case Platform.X:
                return {
                    ...
                    brandTextColor: "text-[#2b2b2f]",
                    brandBgColor: "bg-slate-100",
                    brandBorderColor: "border-slate-200",
                    activeColor: "#2b2b2f"
                };
            case Platform.Instagram:
                return {
                    ...
                    extra: (gIdx: number, iIdx: number) => (
                        ...
                        >
                            <span className={`w-1.5 h-1.5 rounded-full transition-colors ${includeFooter ? 'bg-white shadow-[0_0_8px_white]' : 'bg-slate-300'}`} />
            ...
            case Platform.GoogleMaps:
                return {
                    icon: (
                        <div className="w-10 h-10 bg-slate-50 shadow-sm rounded-2xl flex items-center justify-center border border-slate-100">
                            <svg className="w-6 h-6 text-[#2b2b2f]" viewBox="0 0 24 24" fill="currentColor">
                    ...
                    brandBorderColor: "border-[#4285F4]/10",
                    activeColor: "#4285F4"
                };
            case Platform.Line:
                return {
    ...
    const getTabIcon = (p: Platform, isSelected: boolean) => {
        ...
        switch (p) {
            ...
            case Platform.X:
                return (
                    <svg className={iconClass} viewBox="0 0 24 24" fill={isSelected ? "#2b2b2f" : "currentColor"}>
                        ...
                        {results.length > 0 && (
                            <div className="flex items-center w-full border-b border-slate-50 bg-slate-50/50 py-2 px-2 overflow-x-auto no-scrollbar">
                                {results.map((res, idx) => {
                                    ...
                                    const activeColor = (theme as any).activeColor || "#2b2b2f";
                            ...
                            {results.length === 0 ? (
                                <div className="p-12 h-full flex flex-col items-center justify-center text-center space-y-10 animate-in fade-in zoom-in-95 duration-1000">
                                    <div className="relative">
                                        <div className="w-32 h-32 rounded-[42px] bg-slate-50 flex items-center justify-center text-[#2b2b2f] mx-auto relative z-10 border border-slate-100">
                                            <SparklesIcon className="w-16 h-16 animate-pulse-gentle text-[#FFD166]" />
                                        </div>
                                        <div className="absolute inset-0 bg-[#80CAFF]/10 blur-[40px] rounded-full animate-pulse-slow scale-150 -z-10" />
                                    </div>
                                    <div className="space-y-4 max-w-sm">
                                        <h2 className="text-xs tracking-[0.4em] font-black text-[#C084FC] uppercase">Ready to Shine</h2>
                                        <h3 className="text-3xl font-black text-[#2b2b2f] tracking-tighter">AIが最高の投稿を<br />準備しています</h3>
                                                        ...
                                                        <div className={`mb-2 relative group/textarea ${theme.wrapperClass || ''}`}>
                                                            {text ? (
                                                                <AutoResizingTextarea
                                                                    ...
                                                                    className={`w-full bg-transparent focus:outline-none resize-none placeholder:text-slate-300 whitespace-pre-wrap overflow-visible ${theme.contentClasses || 'text-base text-[#2b2b2f] font-bold'}`}
                                                                    trigger={activeTab}
                                                                />
                                                            ) : (
                                                                <div className="py-4 text-slate-400 italic">コンテンツがありません</div>
                                                            )}
                                                        </div>
                                                        ...
                                                        <div className="mt-auto pt-10 border-t border-slate-100">
                                                            <div className="flex items-center justify-between mb-8 gap-3">
                                                                ...
                                                                <div className="flex items-center gap-2">
                                                                    <button
                                                                        onClick={() => onCopy(text)}
                                                                        className="flex items-center justify-center w-12 h-12 rounded-2xl bg-slate-50 text-slate-400 border border-slate-100 hover:bg-slate-100 hover:text-[#2b2b2f] transition-all active:scale-[0.92] shadow-sm"
                                                                        title="コピー"
                                                                    >
                                                            ...
                                                            <div className="flex flex-col gap-4">
                                                                <button
                                                                    ...
                                                                    className="flex items-center justify-center gap-3 py-5 rounded-full text-[13px] font-black tracking-widest transition-all duration-300 relative group bg-slate-50 text-slate-500 border border-slate-100 active:scale-[0.98] shadow-sm hover:border-slate-300 hover:text-[#2b2b2f] overflow-hidden"
                                                                >
