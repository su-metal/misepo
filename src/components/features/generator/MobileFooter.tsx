...
const CustomHome = ({ active }: { active: boolean }) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M3 10L12 3L21 10V20C21 20.5523 20.5523 21 20 21H15V14H9V21H4C3.44772 21 3 20.5523 3 20V10Z"
            stroke={active ? "var(--plexo-yellow)" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
            style={{ opacity: active ? 1 : 0.4 }}
            fill={active ? "var(--plexo-yellow)" : "none"} fillOpacity={active ? 0.2 : 0} />
    </svg>
);
...
const CustomHistory = ({ active }: { active: boolean }) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M3 12C3 16.9706 7.02944 21 12 21C16.9706 21 21 16.9706 21 12C21 7.02944 16.9706 3 12 3C9.5 3 7.25 4.01 5.6 5.6"
            stroke={active ? "var(--plexo-yellow)" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : 0.4 }} />
        <path d="M5.6 1.6V5.6H1.6" stroke={active ? "var(--plexo-yellow)" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : 0.4 }} />
        <path d="M12 7V12L15 15" stroke={active ? "var(--plexo-yellow)" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : 0.4 }} />
    </svg>
);

const CustomAvatar = ({ active }: { active: boolean }) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="12" cy="7" r="4" stroke={active ? "var(--plexo-yellow)" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : 0.4 }} />
...
const CustomSettings = ({ active }: { active: boolean }) => (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <circle cx="12" cy="12" r="3" stroke={active ? "var(--plexo-yellow)" : "white"} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ opacity: active ? 1 : 0.4 }} />
...
export const MobileFooter: React.FC<MobileFooterProps> = ({
    ...
    return (
        <div className="sm:hidden fixed bottom-0 left-0 right-0 z-[150] px-6 pb-8 bg-transparent pointer-events-none">
            {/* The Floating Bar Background - Glassmorphism */}
            <div className="relative h-20 w-full flex items-center justify-between px-7 bg-white/10 backdrop-blur-2xl rounded-[40px] overflow-visible pointer-events-auto border border-white/10 shadow-[0_20px_40px_rgba(0,0,0,0.2)]">
                ...
                <div className="absolute top-[-32px] left-1/2 -translate-x-1/2 w-[88px] h-[88px] z-10 pointer-events-none">
                    <svg viewBox="0 0 88 88" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
                        <circle cx="44" cy="34" r="32" fill="#6339f9" fillOpacity="0.8" />
                    </svg>
                </div>
                ...
                <button
                    ...
                    className={`
                        ...
                        shadow-[0_15px_30px_rgba(0,0,0,0.3)] active:scale-90 transition-all duration-300 border-4 border-white/10
                        ${isConfirmStep
                            ? 'bg-[var(--plexo-yellow)] text-[var(--plexo-black)] rotate-0 scale-110 shadow-lg border-[var(--plexo-yellow)]'
                            : 'bg-white rotate-180 scale-100 border-white/20'
                        }
                    `}
