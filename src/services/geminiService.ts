...
export const generateInspirationCards = async (
  ...
  4. 「勇気」「希望」などの精神論は禁止。
  5. **重要**: outputの 'prompt' フィールドは、AIに対する「〜する記事を書いて」という**命令形**の指示にすること。「〜はいかがですか？」などの疑問形は禁止。

  【出力データの定義】
  - title: 提案のタイトル (10文字以内)
  - description: 提案の概要 (ユーザーへの提案文)
  - prompt: **記事作成AIへの具体的な指示文** (例: "新商品の魅力を伝える記事を書いてください") 
  - icon: 絵文字
