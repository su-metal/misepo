...
【執筆ルール（最優先）】
- **完全継承**: 語順・改行リズム・絵文字密度、そして「語尾のニュアンス」を極限まで模倣。サンプルにない「〜ですよ」「〜ですね」といったAI特有の親しみやすさの追加は厳禁。
- **癖の抽出**: サンプルに特有の口癖がある場合のみ再現し、サンプルが標準的な「です・ます」であれば一切のアレンジを加えずそのまま出力する。
- **脱テンプレート**: サンプルの「話の構成・順序」は模倣せず、メモの内容に合わせて構成する。
