...
<system_instruction>
  ...
  <rules>
    ...
    - Features: ${isInstagram ? 'Visual focus. **IMPORTANT: HOOK FIRST**. You MUST include key information (Price, Discounts, or a strong Benefit) within the **first 3 lines** so it is visible before the "More" button on Instagram. **CRITICAL: MANDATORY HASHTAGS**. You MUST include exactly **3 to 5 relevant hashtags** at the very end of the post. No more than 5. Insert exactly **two empty lines** between the body text and the hashtag block.' : ''}${isX ? 'Under 140 chars. **IMMEDIACY & VOICE**: Prioritize real-time "now" moments (e.g., "Just cooked", "Last few left"). Focus on conversational engagement. Use exactly 1-2 relevant hashtags for discoverability.' : ''}${isGMap ? `NO hashtags. Focus on maintaining the owner's personality in the reply. ${GMAP_NEGATIVE_CONSTRAINTS}` : ''}${isLine ? 'Direct marketing style. NO hashtags. Focus on clear messaging.' : ''}
