...
const getModelName = (isPro: boolean) => {
  // Use Flash for speed and efficiency for text tasks
  return "gemini-2.5-flash";
};

// Comprehensive Symbol Palette
// NOTE: Cleaned of invisible control characters (U+202C, U+200E) to prevent JSON parsing errors.
...
// Helper to check character count (JS length, where Emoji is usually 2, Kanji is 1)
const getLength = (text: string) => text.length;

// --- Risk Scoring Logic (Deterministic) ---
const KEYWORDS = {
...
`;
  ...
  if (config.instagramFooter) {
    // IMPORTANT: Treat as context but do not append in the output from AI. Frontend will append it.
  ...
  // Determine emoji usage: Default to true unless explicitly set to false (Free plan sends undefined, Pro sends boolean)
...
5. ${config.platform === Platform.X && config.xConstraint140 ? 'MUST be under 140 characters.' : ''}
...
`;
  ...
  const userPrompt = `
Draft a post based on this input: "${config.inputText}"
`;
...
export const analyzeRisk = async (starRating: number, reviewText: string): Promise<RiskAnalysisResult> => {
    // 1. Rule-based scoring (Fast, Deterministic)
    const baseAnalysis = scoreRisk(starRating, reviewText);

    // If score is high/critical, optionally use AI to verify (omitted for speed/cost in this demo, trusting rule-based)
    return baseAnalysis;
};
