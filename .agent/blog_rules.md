...
### 1-1. 基本設定
- **ターゲット読者**: 個人店の店主（カフェ、美容室など）、SNS運用に苦手意識のある小規模事業主。
- **口調・トーン**: 専門家として教えるのではなく、読者（店主様）に寄り添い、一緒に歩む「温かい伴走者」のような語り口。専門用語は避け、等身大で親しみやすい言葉を使います。
- **文字数目安**: 1,500文字 〜 2,500文字程度（SEO効果と読後感のバランスを重視）。
- **記事の構成**: 
    1. **導入**: 読者の現状の不満や悩みに深く共感。この記事を読めば「何が変わるか」を力強く提示。
    2. **メイン（4〜5つのポイント）**: 単なる一般論ではなく、具体的なシチュエーション（来店、予約、口コミ、写真撮影など）に基づいた解決策を4〜5項目で詳しく解説。
...
# 厳守ルール（記号・表記） ※最重要
- **Markdown記法を一切使わない**（`#` や `###` も含むすべてのMarkdown記号が禁止です。見出しは「■」などの記号で代用してください）。
