...
export async function POST(req: Request) {
  try {
    ...
    // ---- intro discount (first-month coupon) - monthly only
    ...
    // ---- 7-day trial (recommended: collect payment method upfront)
    // We implement trial via subscription_data.trial_period_days (not via Price UI).
    // Make it "first time only" by reusing promotion_redemptions with a separate key.
